...
- **Manifest and runtime.** `packaging/windows/AppxManifest.xml` carries a
  package-version placeholder that `ci/stage-msix.sh` substitutes at build
  time. The workflow bundles the Visual C++ runtime DLLs app-locally in the
  MSIX instead of declaring a Store framework package dependency. Keep the
